...
使用 `ip -6 addr` 发现地址没有被分配上，`systemctl status systemd-networkd` 查看日志，发现被加载的是 `20-ethernet.network`，这里我使用 `cp 20-wired.network 20-ethernet.network` 来覆盖掉默认的配置文件，重新启动 `systemd-networkd` 后，地址分配成功
